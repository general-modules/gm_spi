 ...
 * @brief SPI 片选控制
 *
 * @param[in] gm_spi: SPI 对象
 * @param[in] enable: 是否使能片选 (true: 使能, false: 失能)
 *
 * @return 0 : 成功
 * @return <0: 失败
 */
static int gm_spi_cs_control(const gm_spi_t *gm_spi, const bool enable)
{
    if (gm_spi == NULL)
    {
        return -1;
    ...
    if (gm_spi->cs_control_cb != NULL)
    {
        return gm_spi->cs_control_cb(enable);
    }
...
{
    gm_spi_t *gm_spi = (gm_spi_t *)malloc(sizeof(gm_spi_t));
    if (gm_spi == NULL)
    {
        return NULL;
    }
    ...
    gm_spi->fd = -1;
    gm_spi->cs_control_cb = NULL;
    gm_spi->max_transfer_len = 0;
    pthread_mutex_init(&gm_spi->mutex, NULL);
                ...
                const uint8_t spi_bits)
{
    ...
    pthread_mutex_lock(&gm_spi->mutex);
    if (gm_spi->fd != -1)
    {
        close(gm_spi->fd);
        gm_spi->fd = -1;
    ...
    if (fd < 0)
    {
        pthread_mutex_unlock(&gm_spi->mutex);
    ...
    gm_spi->fd = fd;
    gm_spi->max_transfer_len = gm_spi->max_transfer_len == 0 ? 4096 : gm_spi->max_transfer_len;
    pthread_mutex_unlock(&gm_spi->mutex);
...
int gm_spi_destroy(gm_spi_t *gm_spi)
{
    ...
    pthread_mutex_unlock(&gm_spi->mutex);
    pthread_mutex_destroy(&gm_spi->mutex);
    free(gm_spi);
...
int gm_spi_set_cs_control_cb(gm_spi_t *gm_spi, gm_spi_cs_control_cb cs_control_cb)
{
    ...
    pthread_mutex_lock(&gm_spi->mutex);
    gm_spi->cs_control_cb = cs_control_cb;
    pthread_mutex_unlock(&gm_spi->mutex);
...
int gm_spi_set_max_transfer_len(gm_spi_t *gm_spi, const size_t max_transfer_len)
{
    ...
    pthread_mutex_lock(&gm_spi->mutex);
    gm_spi->max_transfer_len = max_transfer_len == 0 ? 4096 : max_transfer_len;
    pthread_mutex_unlock(&gm_spi->mutex);
...
int gm_spi_write_data(gm_spi_t *gm_spi, const uint8_t *write_data, const size_t write_data_len)
{
    ...
    if (gm_spi_cs_control(gm_spi, true) < 0)
    {
        pthread_mutex_unlock(&gm_spi->mutex);
    ...
    {
        // 本次发送数据长度
        size_t current_len = remain_data_len > gm_spi->max_transfer_len ? gm_spi->max_transfer_len : remain_data_len;
        // 本次发送数据偏移量
        ...
        if (ioctl(gm_spi->fd, SPI_IOC_MESSAGE(1), &spi_transfer) < 0)
        {
            gm_spi_cs_control(gm_spi, false);
            pthread_mutex_unlock(&gm_spi->mutex);
                          ...
                          const size_t write_data_len)
{
    ...
    while (remain_data_len > 0)
    {
        ...
        if (remain_data_len == write_data_len)
        {
            ...
            ret = ioctl(gm_spi->fd, SPI_IOC_MESSAGE(2), &spi_transfer);
        }
        // 后续数据不需要发送寄存器地址
        else
        {
            ...
            ret = ioctl(gm_spi->fd, SPI_IOC_MESSAGE(1), &spi_transfer[0]);
        }
